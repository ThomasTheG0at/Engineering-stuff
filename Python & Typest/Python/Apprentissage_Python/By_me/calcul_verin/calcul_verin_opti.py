...
def demander_float(message):
    while True:
        try:
            valeur = input(message).replace(" ", "").replace(",", ".")
            return float(valeur)
        except ValueError:
            print("Saisie invalide. Merci d'entrer un nombre valide (ex: 12.5 ou 12,5).")

def saisie_donnees_verin():
    D = demander_float("Quel est le diamètre D du piston (en mm) ? : ")
    d = demander_float("Quel est le diamètre d de la tige (en mm) ? : ")
    P = 0.0
    F = 0.0
    choix = input("Connaissez-vous : (1) La pression d'utilisation ou (2) La force souhaitée ?")
    if choix == "1": 
        P = demander_float("Quel est la pression P d'utilisation (en bar) ? : ")
    elif choix == "2":
        F = demander_float("Quel est la force souhaitée F (en N) ? : ")
    return D, d, P, F
...
def saisie_conditions_utilisation():
    ...
    if choix == "1": 
        Q = demander_float("Quel est le débit Q de la pompe (en m³/s) ? : ")
    elif choix == "2":
        V = demander_float("Quel est la vitesse de sortie V souhaitée (en m/s) ? : ")
    F_ext = demander_float("Quel est la charge extérieure réelle à déplacer (en N) ? : ")
    return Q, V, F_ext
